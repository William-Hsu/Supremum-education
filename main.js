...
/* ── 15. SCROLL PROGRESS ALONG THE SUPREMUM CURVE ── */
// The dot travels the curve as you scroll; reaches the limit L at page bottom.
(function () {
  const path = document.getElementById('progressPath');
  const dot  = document.getElementById('progressDot');
  const wrap = document.getElementById('curve-progress');
  ...
  function update() {
    ...
    // Glow brighter as you approach the limit
    dot.setAttribute('r', 4 + t * 1.5);
    wrap.classList.toggle('at-limit', t > 0.98);
    ticking = false;
  }
...
/* ── 16. CURVE SPINE — spans hero-bottom to CTA-top ── */
(function () {
  const spine = document.getElementById('curve-spine');
  const path  = document.getElementById('spinePath');
  const glow  = document.querySelector('.spine-glow');
  const dotEl = document.getElementById('spineDotEl');
  if (!spine || !path) return;
  ...
  let spineStart = 0, spineEnd = 0;

  function measure() {
    // Start below the hero so the line never crosses the credentials panel
    const hero = document.querySelector('#home') || document.querySelector('.page-hero');
    const cta  = document.querySelector('.cta-section');
    spineStart = hero ? hero.offsetTop + hero.offsetHeight : 0;
    spineEnd   = cta  ? cta.offsetTop : document.body.scrollHeight;
    spine.style.top    = spineStart + 'px';
    spine.style.height = Math.max(spineEnd - spineStart, 0) + 'px';
    update();
  }

  let ticking = false;
  function update() {
    const span = spineEnd - spineStart;
    if (span <= 0) { ticking = false; return; }
    // Dot rides at ~65% of viewport height, not the bottom edge
    const anchor = window.scrollY + window.innerHeight * 0.65;
    const p = Math.min(Math.max((anchor - spineStart) / span, 0), 1);
    ...
    if (dotEl) {
      ...
      dotEl.style.left = (pt.x / 100  * spine.clientWidth)  + 'px';
      dotEl.style.top  = (pt.y / 1000 * spine.clientHeight) + 'px';
      // Hide the dot until the spine zone is actually in view
      dotEl.style.opacity = (p > 0.001 && p < 0.999) ? '1' : (p >= 0.999 ? '1' : '0');
    }
    spine.classList.toggle('at-limit', p > 0.985);
    ticking = false;
  }
