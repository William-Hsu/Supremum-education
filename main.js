...
/* ── 16. CURVE SPINE — full-width weave, HTML dot, ends at CTA ── */
(function () {
  ...
  function measure() {
    // Spine ends where the CTA section begins, so they never overlap
    const cta = document.querySelector('.cta-section');
    spineEnd = cta ? cta.offsetTop : document.body.scrollHeight;
    spine.style.height = spineEnd + 'px';
    update();
  }

  let ticking = false;
  function update() {
    if (spineEnd <= 0) { ticking = false; return; }
    const p = Math.min(Math.max((window.scrollY + window.innerHeight) / spineEnd, 0), 1);
    const offset = total * (1 - p);
    path.style.strokeDashoffset = offset;
    if (glow) glow.style.strokeDashoffset = offset;

    if (dotEl) {
      // Convert viewBox coords (100 x 1000) to real pixels — keeps the dot circular
      const pt = path.getPointAtLength(total * p);
      const w  = spine.clientWidth;
      const h  = spine.clientHeight;
      dotEl.style.left = (pt.x / 100  * w) + 'px';
      dotEl.style.top  = (pt.y / 1000 * h) + 'px';
    }
