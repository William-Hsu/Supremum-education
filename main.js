...
/* ── 16. CURVE SPINE — scroll-linked drawing down the page ── */
(function () {
  const spine = document.getElementById('curve-spine');
  const path  = document.getElementById('spinePath');
  const dot   = document.getElementById('spineDot'); // optional
  ...
  function update() {
    ...
    const p = Math.min(Math.max((window.scrollY + window.innerHeight) / docH, 0), 1);
